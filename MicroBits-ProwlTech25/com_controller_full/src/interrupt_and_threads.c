...
// Variabel for mellomlagring av joystick-koordinater etter stereografisk projeksjon
struct ProjectionResult x_y_temp;

// Global variabel for sensorverdi mottatt over CANBUS
uint8_t sensorRead;

// Trådstruktur-variabler for hver tråd
struct k_thread process_thread1;
struct k_thread process_thread2;
struct k_thread process_thread3;

// Variabel for rå joystick-data mottatt over Bluetooth
static struct joystick_data js_raw_data;

// Variabel for kjørekommando
struct data_for_motorcontroller data;

// Tråd-ID variabler (NULL = ikke startet ennå)
k_tid_t js_thread_id = NULL;
k_tid_t sensor_thread_id = NULL;
static k_tid_t thread_id = NULL;

// Variabel for feilsøking – teller hvor mange ganger interrupt er trigget
...
// Behandler data og sender kommando til motorcontroller
void process_data(struct data_for_motorcontroller data, uint8_t sensorVals) {
    ...
    // Venter til tråden er ferdig og frigjør ressursene
    if (thread_id != NULL) {
...
// Returnerer interrupt-telleren for feilsøking
int get_counter() {
    return counter;
}

// Timer-handler som starter prosesseringstråd ved interrupt
static void timer_handler(nrf_timer_event_t event_type, void *p_context) {
    if (event_type == NRF_TIMER_EVENT_COMPARE0) {
        if (thread_id == NULL) {
            counter++;

            // Starter en ny tråd for prosessering
...
void continous_js_thread() {
    while (1) {
        ...
        // Sover før ny iterasjon
        k_msleep(10);
    }
}

// Kontinuerlig tråd for innhenting av sensor-data
...
// Initialiserer og starter timerbasert interrupt
